...
import org.openqa.selenium.devtools.v136.page.Page.CaptureScreenshotFormat;
...
//		java.io.File srcFile = ts.getScreenshotAs(OutputType.FILE);
		...
		// Timestamp to make file unique
		String timestamp = new SimpleDateFormat("yyyyMMdd_HHmmss").format(new Date());

		// Build dynamic folder path
		String destDir = System.getProperty("user.dir") + "/" + folderName + "/";
		String destPath = destDir + fileName + "_" + timestamp + ".png";

		try {
			TakesScreenshot ts = (TakesScreenshot) driver;
			java.io.File srcFile = ts.getScreenshotAs(OutputType.FILE);
			
            System.out.println("Screenshot saved at: " + srcFile);
			
			
		} catch (Exception e) {
			System.out.println("Issue in to take screenshot :" + e);
		}
